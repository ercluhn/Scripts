...
import cPickle, os,sys, stat, shutil #imports those things
...
class poser():	  
	...
	def writeFile(self,*args):
		...
		selection = cmds.ls (sl = True) #gets names of selected objects
		...
		if selection == []: #if selection is empty, raises a warning
				...
				if poseFile in poses: # if a file with the same name already exists, raises a warning.
					...
					if (self.fileNameQuery =='') or (self.fileNameQuery == 'Please enter desired file name'):
						...
						if not self.folderQuery: #if there's nothing selected, raises a warning
							...
							fileOpen = open(self.fileLoc + '/Characters/' +self.folderQuery[0] + '/POSE_' + self.fileNameQuery + '.txt', "w+") #opens the file
	...
	def readFile(self,*args): #read file
		...
		if not self.folderQuery :
			...
			if self.poseName:
				fileOpen = open (self.fileLoc + '/Characters/' + self.folderQuery[0] + '/' + self.poseName[0], 'r')
				sData = fileOpen.read()
